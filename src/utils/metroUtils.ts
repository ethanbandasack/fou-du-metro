  ...
  const createVariations = (name: string) => {
    ...
    return [
      ...
      normalized
        .replace(/[-]/g, " ")
        .replace(/[^a-z0-9\s]/g, "")
        .replace(/\s+/g, ""), // Convert hyphens to spaces then remove spaces
    ];
  };
 ...
 */
export function getLinesByType(
  stations: MetroStation[],
): Record<string, Record<string, MetroStation[]>> {
  ...
  Object.keys(linesByType).forEach((type) => {
    Object.keys(linesByType[type]).forEach((line) => {
      linesByType[type][line].sort((a, b) =>
        a.nom_long.localeCompare(b.nom_long),
      );
    });
  });
